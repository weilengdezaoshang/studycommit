...
import { NotebookSelect } from '../notebook/NotebookSelect'
import './notebook-drawer.css'
...
function DrawerIdentity({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  const status = session ? '已登录 · 云端同步' : '未登录 · 本机记录'
  ...
  return (
    ...
    >
      ...
      <div className="drawer-month-nav">
        ...
        <div className="drawer-month-selects">
          <NotebookSelect
            aria-label="选择年份"
            value={cursor.year}
            onValueChange={(value) => {
              setCursor((previous) => ({ ...previous, year: Number(value) }))
              setHelpOpen(false)
            }}
          >
            {Array.from({ length: CALENDAR_YEAR_COUNT }, (_, index) => {
          ...
          <NotebookSelect
            ...
            value={cursor.month}
            onValueChange={(value) => {
              setCursor((previous) => ({ ...previous, month: Number(value) }))
              setHelpOpen(false)
            }}
          >
          ...
          {week.map((cell) => {
            ...
            return (
              <button
                ...
                aria-current={cell.dateKey === today ? 'date' : undefined}
                title={`${cell.dateKey} · ${cell.count} 条记录`}
                onClick={() => {
                ...
                className={[
                  ...
                  .join(' ')}
              >
                {cell.isSelected && <SketchBorder />}
                <span className="calendar-date-number">{Number(cell.dateKey.slice(8))}</span>
                <span className="calendar-record-count" aria-hidden="true">
                  {cell.count > 0 ? `${cell.count}条` : ''}
                </span>
              </button>
            )
          })}
      ...
      {helpOpen && (
        <p className="drawer-month-help" role="note">
          日期下方显示当天记录条数，圆圈标记今天，手绘方框表示选中日期。点日期查看当天记录，未来日期不可选。
        </p>
      )}
    </div>
  )
...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        id="study-drawer"
        className={`sidebar notebook-drawer ${open ? 'sidebar--open' : ''}`}
        aria-hidden={!open}
        inert={!open}
        onKeyDown={(event) => {
