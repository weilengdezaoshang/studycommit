...
test('尚无价格版本时可以显式配置模型并发布首个价格', async ({ page }) => {
  let posted: { configSnapshot: { model: string; estimatedCostPerRun: string } } | null = null
...
return route.fulfill({ json: aiProvider })
    ...
    if (path.endsWith('/ai/prices')) {
      if (route.request().method() === 'POST') {
        posted = route.request().postDataJSON() as NonNullable<typeof posted>
  ...
  expect(posted.configSnapshot.model).toBe('approved-model')
  expect(posted.configSnapshot.estimatedCostPerRun).toBe('0.010000')
})
...
return route.fulfill({ json: { items: [] } })
  ...
  expect(saved).toMatchObject({ model: 'gpt-test', reason: '接入平台测试密钥' })
})
