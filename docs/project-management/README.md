...
M2 的 P0 前置机制——应用连接生命周期、前后端副作用清理、租约恢复与故障测试——见
[M2 连接生命周期与异常副作用清理开发文档](M2_CONNECTION_LIFECYCLE_DEVELOPMENT_GUIDE.md)。
...
| M2 学习闭环   | 异常可恢复地完成一次学习并保存结果 | 连接生命周期 → 专题 → 计时 → 收尾 → Learning Log |
